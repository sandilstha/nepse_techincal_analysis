...
(function () {
  ...
  function scatterSvg(pts) {
    var W = 1360, H = 560, m = { l: 56, r: 28, t: 20, b: 44 };
    ...
    [0, 20, 40, 60, 80, 100].forEach(function (v) {
      ...
      svg += "<text x='" + X(v) + "' y='" + (m.t + ih + 18) + "' class='ms-sc-tick' text-anchor='middle'>" + v + "</text>";
    ...
    svg += "<text x='16' y='" + (m.t + ih / 2) + "' class='ms-sc-axis' text-anchor='middle' " +
      "transform='rotate(-90 16 " + (m.t + ih / 2) + ")'>Value score</text>";
    ...
    function labelY(x, y) {
      ...
      while (tries < 30 && placed.some(function (p) {
        return Math.abs(p.x - x) < 62 && Math.abs(p.y - ly) < 13;
      })) { ly -= 13; tries++; if (ly < m.t + 10) { ly = y + 18; } }
    ...
    pts.forEach(function (r) {
      ...
      svg += "<circle cx='" + x + "' cy='" + y + "' r='5' class='ms-sc-dot " + cls + "'>" +
  ...
  function drawScatter(data) {
    ...
    var TIERS = [
      ...
      { key: "—", label: "UNCLASSIFIED (no market cap)" }
    ];
    var html = "";
    TIERS.forEach(function (tier) {
      var group = pts.filter(function (r) { return (r.size || "—") === tier.key; });
      if (!group.length) return;
      html += "<div class='dsx-card ms-scatter-card'>" +
        "<div class='dsx-card-head neutral dsx-ad-head'>" +
        "<span>" + tier.label + " — GROWTH vs VALUE</span>" +
        "<span class='dsx-ad-sub'>" + group.length +
        " companies · right = stronger growth, up = better value · amber = 4★/5★, red = quality-gated</span>" +
        "</div>" + scatterSvg(group) + "</div>";
    });
    host.innerHTML = html;
  }
