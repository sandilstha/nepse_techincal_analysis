 ...
 * The Company tab embeds Stock 360's fundamentals module (fundamentals.js),
 * which OWNS the fa-* ids and reads window.FA_CONFIG. Two collisions existed
 * before this rename and both were silent: this desk's sector <select> is
 * #fa-sector, which that module treats as its sector LABEL span; and its
 * FA_CONFIG.matrixUrl pointed at the industry endpoint, so the company matrix
 * would have queried the wrong API.
 *
...
(function () {
  ...
  function initTabs() {
    ...
    [].forEach.call(tabs, function (btn) {
      btn.addEventListener("click", function () {
        ...
        ["industry", "smart", "company"].forEach(function (name) {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    /* Keep the "Open full Stock 360" link in step with whatever the Company tab
       is showing. fundamentals.js writes the loaded ticker into #fa-ticker, so
       observing that element avoids reaching into its internals. */
    var tick = el("fa-ticker"), open360 = el("fa-open360");
    if (tick && open360 && window.MutationObserver) {
      new MutationObserver(function () {
        var sym = (tick.textContent || "").trim().toUpperCase();
        open360.href = sym && sym !== "—" ? "/stock/" + encodeURIComponent(sym) + "/#fund"
                                          : "/stock/";
