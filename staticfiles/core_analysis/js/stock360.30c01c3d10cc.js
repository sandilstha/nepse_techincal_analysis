  ...
  function renderHistory(hist) {
    ...
    var row = function (label, key) {
      return "<tr><th>" + label + "</th>" + hist.map(function (h) {
        var v = h[key];
        var cls = "num" + (h.pending ? " pend" : "") + (key === "dps" && v > 0 ? " paid" : "");
        var txt = h.pending && key === "dps" ? "—" : pctOr(v);
        return '<td class="' + cls + '">' + txt + "</td>";
      }).join("") + "</tr>";
    };
    return '<div class="dv-hist-wrap"><table class="dv-hist">' +
      "<thead><tr><th>FY</th>" + hist.map(function (h) {
        return "<th" + (h.pending ? ' class="pend" title="In progress — not yet declared"' : "") + ">" +
