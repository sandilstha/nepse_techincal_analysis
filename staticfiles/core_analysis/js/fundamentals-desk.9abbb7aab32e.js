...
(function () {
  ...
  function initTabs() {
    var tabs = [].slice.call(document.querySelectorAll(".dsx-tabs .dsx-tab"));
    /* Derived from the markup, not a hardcoded list — the Stock 360 tab was
       added without touching this function, and the next one should be too. */
    var names = tabs.map(function (b) { return b.getAttribute("data-tab"); });

    function show(want) {
      tabs.forEach(function (b) {
        b.classList.toggle("active", b.getAttribute("data-tab") === want);
      });
    ...
    tabs.forEach(function (btn) {
      btn.addEventListener("click", function () {
        ...
        show(want);
        /* Deep-linkable without a reload, so a shared URL reopens the same
           desk. replaceState, not pushState: tab flips are not navigation. */
        try {
    ...
    if (initial && names.indexOf(initial) !== -1) show(initial);
  }
